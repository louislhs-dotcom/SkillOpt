...
def tdai_read_context(query="moli queries generation prompts research"):
    try:
...
def tdai_write_memory(content):
    ...
    returns 200 but the content is invisible to search (same bug fixed in
    hermes_prompt adapter, 2026-08-18).
    """
    try:
        from skillopt.envs.tdai_atomic_write import write_skillopt_memory
        return write_skillopt_memory(content, "skillopt-moli")
    except Exception:
        return False
...
class MoliAdapter(EnvAdapter):
    def __init__(self, split_dir="", data_path="", split_mode="split_dir",
        ...
        self.dataloader = MoliDataLoader(
    ...
    def rollout(self, env_manager, skill_content, out_dir, **kwargs):
        ...
        if tdai_ctx:
            system_msg = (
                ...
                f"Follow the skill guidelines to choose the right Moli commands, dump formats, and wait strategies.\n\n"
                f"SKILL:\n{skill_content}\n\n"
                f"{tdai_ctx}"
            )
        else:
